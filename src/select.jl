...
NUM_QUERIES = ENV["NUM_QUERY"]
TOP_RESULTS = ENV["TOP_RESULTS"]
...
function get_embeddings(selector::Selector)
    ...
    model = sentence_transformers.SentenceTransformer(selector.model_name, trust_remote_code=true)
    ...
    queries = Corpus([StringDocument(readline(), TextAnalysis.DocumentMetadata(Languages.English(), "Query $n")) for n in 1:selector.num_queries])
...
function evaluate_relevance(
    ...
    for query, query_embed in zip(queries, query_embeds)
    	cos = [compute_cosm(query_embed, view(doc_embeddings, row, :)) for row in 1:size(doc_embeds, 1)] # compute similarity row by row along first dimension
	...
	# Organize results
	df = DataFrame(
	    "queries" = [query for 1:selector.top_results],
	    "titles" = titles,
	    "scores" = scores)
	push!(dfs, df)	
    end
